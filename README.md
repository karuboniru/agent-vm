...
CWD is shared at its canonical absolute path. `--workdir` changes the guest working directory; `--cwd-mode ro|rw|none` changes the default CWD sharing policy. Additional bind mounts default to read-only. CWD reuses an existing mount at the same path and source (and the built-in `/usr` tree), retaining that mount's permissions. `--cwd-mode` only controls the default CWD mount: `--cwd-mode ro` does not downgrade an explicit `rw` mount reused for CWD, and `--cwd-mode rw` does not upgrade an explicit `ro` mount. A conflicting bind or tmpfs at the CWD target is replaced by CWD sharing. Other duplicate mount targets are errors. Use TOML for source paths containing commas, which are separators in `--mount` syntax.

Bind mounts may use new paths beneath `/run`, `/tmp`, `/var/tmp`, `/mnt`, and `/media`, including a CWD on removable storage at `/run/media/<user>/<device>/...`. `/run` itself and ancestors of or the exact `/run/user/<uid>` directory remain protected because the guest manages that directory's permissions. Bind mounts and custom tmpfs mounts can also use `/etc` and `/usr` subpaths, except `/etc/resolv.conf`. Beneath the implicit read-only `/usr` mount, targets must already exist; mount a tmpfs or writable bind at an existing directory without symlink components to create further child mountpoints. `/proc`, `/sys`, `/dev`, FHS aliases (`/bin`, `/sbin`, `/lib`, `/lib64`), and internal bootstrap paths remain reserved for filesystem assembly.
...
`--network none` disables external networking and TSI. Guest-local loopback remains available. Fixed, explicit vsock channels remain enabled for signals, terminal resizing and a dedicated one-shot readiness notification. Explicit socket forwarding works with either network mode.
...
`--profile NAME` (or `--profile=NAME`) replaces the default filename with `NAME.toml` in the same configuration directory. For example, `agent-vm run --profile test` loads `$XDG_CONFIG_HOME/agent-vm/test.toml`, or `~/.config/agent-vm/test.toml` when `XDG_CONFIG_HOME` is unset, empty or relative. Profile names must be non-empty and cannot contain path components. The selected file must exist; it replaces rather than merges with `config.toml`. `--config`, `--profile` and `--no-config` are mutually exclusive.
...
The generated `/etc` contains minimal account/NSS/host configuration. Timezone, CA certificates and the loader cache are imported from fixed system locations. Host `/etc/ld.so.conf` and the contents of `/etc/ld.so.conf.d/` are copied into the private root when present; symlinks to regular configuration files are copied as files. These are read-only private files. Host `/etc/alternatives` is bind-mounted read-only when present, preserving its symlinks. `/etc/resolv.conf` is a separate private writable mount for guest DHCP. The whole host `/etc` is never shared automatically. Explicit file and directory binds below `/etc` are supported, for example `--mount src=/etc/machine-id,dst=/etc/machine-id,ro`; their configured `ro`/`rw` modes apply. Replacing `/etc` itself or `/etc/resolv.conf` is prohibited.
...
The guest boots with `oops=panic panic=-1`, so kernel Oopses trigger a panic and panics immediately reset the VM, causing libkrun to exit. The guest helper seeds exit status 125 before filesystem assembly; libkrun's init replaces it with the command's status after the helper finishes exiting. This keeps crashes during mount teardown from being reported as success.

libkrunfw 5.5.0 embeds Linux 6.12.91, which can crash during virtiofs submount teardown with `VFS: Busy inodes after unmount`. Linux 6.12.95 includes the upstream fix, [virtiofs: fix UAF on submount umount](https://cdn.kernel.org/pub/linux/kernel/v6.x/ChangeLog-6.12.95). Use a libkrunfw build containing that fix; upstream libkrunfw 5.6.0 rebases to Linux 6.12.109. The panic settings make this failure exit promptly but do not fix the race.
...
- After starting host helpers and before forking the VMM, the supervisor enters a single-ID user namespace and an empty network namespace, clears capabilities and sets `no_new_privs`. This also applies with networking disabled. Passt stays in the host network namespace; existing sockets and filesystem Unix sockets carry communication across the boundary.
...
- A source `--mask` hides that subtree through each declared shared path, except for explicitly shared descendants (for example, mask `.ssh` and share `.ssh/known_hosts`). The same source or target cannot be both shared and masked. Existing filesystem bind aliases that could bypass the policy are conservatively rejected, including aliases of `/usr` and the private runtime. Target symlinks, missing mask paths under shared trees, and nested mounts requiring directory creation in read-only shares are rejected. `--mask-target` applies to one guest target. `plan` previews configuration; mount identity and pinned-FD checks also run during actual launch.
- A descendant share restored as a source-mask exception remains visible if `--mask-target` also covers that masked directory or an intermediate ancestor of the share. For example, sharing `.ssh/sub/known_hosts` beneath a source-masked `.ssh` is still an allow exception when its guest `.ssh` or `.ssh/sub` directory is target-masked. Sharing a parent directory does **not** cancel masks of its children: sharing `.ssh/allowed` while masking `.ssh/allowed/secret` keeps `secret` hidden, including when `allowed` is itself an exception to a mask of `.ssh`. Other contents of the masked ancestors remain hidden. Masks are not applied as an order-dependent list of CLI overrides.
