...
Nested bind mounts have independent modes: a writable child can sit under a read-only parent, and a read-only child under a writable parent. A parent's read-only setting covers its tree except for separately declared child mounts. Bind mounts and custom tmpfs mounts may be nested together, with parents installed before children regardless of configuration or CLI order. When the nearest enclosing mount is a bind mount, the nested target must already exist in that mount's source, match the child source's file/directory type, and contain no symlink components. When the nearest enclosing mount is a tmpfs, child mountpoints can be created privately. The runner never creates mountpoints inside host shares. For example, a read-only home mount can contain the default writable CWD mount.
